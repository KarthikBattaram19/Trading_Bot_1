...
def test_score_weight_candidates_event_count_and_ordering():
    returns = _simulate_garch_returns(400, gamma=0.05, alpha=0.05, beta=0.90, vl=0.0002, seed=7)
    grid = score_weight_candidates(
        returns,
        candidates=[(0.05, 0.05, 0.90), (0.04, 0.08, 0.88)],
        window=250,
    )
    assert grid.n_events == 400 - 250
    assert grid.default.n_events == 150
    assert len(grid.candidates) == 2
    # Candidates are sorted best-first by mean QLIKE.
    qlikes = [c.mean_qlike for c in grid.candidates]
    assert qlikes == sorted(qlikes)


def test_score_weight_candidates_default_candidate_ties_default():
    returns = _simulate_garch_returns(400, gamma=0.05, alpha=0.05, beta=0.90, vl=0.0002, seed=11)
    grid = score_weight_candidates(
        returns,
        candidates=[(0.05, 0.05, 0.90)],
        window=250,
    )
    same = grid.candidates[0]
    assert same.mean_qlike == grid.default.mean_qlike
    # Identical forecasts never strictly beat the default.
    assert same.win_rate_vs_default == 0.0


def test_score_weight_candidates_true_weights_beat_wrong_default():
    # Series generated by a reactive GARCH; the true weights should score
    # a lower (better) mean QLIKE than a sluggish default.
    returns = _simulate_garch_returns(700, gamma=0.03, alpha=0.15, beta=0.82, vl=0.0002, seed=5)
    grid = score_weight_candidates(
        returns,
        candidates=[(0.03, 0.15, 0.82)],
        window=250,
        default_weights=(0.05, 0.05, 0.90),
    )
    true_cand = grid.candidates[0]
    assert true_cand.mean_qlike is not None and grid.default.mean_qlike is not None
    assert true_cand.mean_qlike < grid.default.mean_qlike
    assert (true_cand.win_rate_vs_default or 0.0) > 0.5
